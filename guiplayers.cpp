...
GUIPlayers::GUIPlayers(Player* p, int playerNum){
    ...
    //player's game piece
    std::string tempPiece = player->getGamePieceName();
...
void GUIPlayers::addProperty(int propertyNum, std::string propertyName, Space** tempArray){
...
void GUIPlayers::upgradeSpace(){
    ...
    if(isUpgraded == true){
        ...
        if((player->getMoneyAmount() - allSpaces[currentPropertyNum]->getPropertyCost()) <= 0) {
            std::string text = "You do not have enough money to upgrade this property!";
        ...
        std::string text = "Maximum (5) amount of upgrades reached for this property!";
        QString qText = QString::fromStdString(text);
